...
#define BATTERY_PADDING_RIGHT 5
#define BATTERY_ICON_SIZE 20
#define BATTERY_ICON_GAP 2

// Accelerometer zone constants for wrist flick detection
// Based on pebble_glancing_demo: watch tilted towards user with screen facing
// user
...
// Daytime hours
#define DAYTIME_START 8
#define DAYTIME_END 18
...
static bool s_current_static_is_playing =
	true; // Track which static frame is loaded
...
{
	return tick_time->tm_hour >= DAYTIME_START &&
	       tick_time->tm_hour < DAYTIME_END;
}

static bool get_current_animation_type()
{
	time_t temp = time(NULL);
	struct tm *tick_time = localtime(&temp);
	return is_daytime(tick_time);
}

static void cancel_animation_timers()
{
	if (s_timer) {
...
static void cleanup_sequence_and_bitmap()
{
	if (s_sequence) {
...
static void load_static_frame(bool is_playing)
{
	uint32_t resource_id = is_playing ? RESOURCE_ID_KITTEN_PLAY_TIME
...
static void load_sequence(uint32_t resource_id)
{
	...
	if (s_sequence) {
		...
		// Always use 8-bit format - APNG decoder outputs 8-bit even for
		// BW platforms
		s_bitmap = gbitmap_create_blank(frame_size, GBitmapFormat8Bit);
...
static void stop_animation_handler(void *context)
{
	...
	// Revert to static first frame
	load_static_frame(get_current_animation_type());
}
...
static void timer_handler(void *context)
{
	...
	if (gbitmap_sequence_update_bitmap_next_frame(s_sequence, s_bitmap,
						      &next_delay)) {
...
static void update_time()
{
	...
	strftime(s_date_buffer, sizeof(s_date_buffer), "%a, %d %b %Y",
		 tick_time);
...
static void accel_data_handler(AccelData *data, uint32_t num_samples)
{
	...
	// Trigger animation when transitioning from inactive to active (wrist
	// flick)
	if (in_active_zone && was_inactive) {
		bool is_playing = get_current_animation_type();
#ifdef PBL_BW
...
static void battery_callback(BatteryChargeState state)
{
	...
	snprintf(battery_buffer, sizeof(battery_buffer), "%d%%",
	...
	if (state.is_charging) {
		s_battery_icon = gbitmap_create_with_resource(
			RESOURCE_ID_BATTERY_CHARGING);
	} else if (state.charge_percent == 100) {
		s_battery_icon =
			gbitmap_create_with_resource(RESOURCE_ID_BATTERY_FULL);
	} else if (state.charge_percent >= 60) {
		s_battery_icon = gbitmap_create_with_resource(
			RESOURCE_ID_BATTERY_HEALTHY);
	} else if (state.charge_percent >= 40) {
		s_battery_icon =
			gbitmap_create_with_resource(RESOURCE_ID_BATTERY_HALF);
	} else {
		s_battery_icon =
			gbitmap_create_with_resource(RESOURCE_ID_BATTERY_LOW);
	}
...
static void init()
{
	...
	// Create BitmapLayer for animation (full screen)
	s_bitmap_layer =
		bitmap_layer_create(GRect(0, 0, IMAGE_WIDTH, IMAGE_HEIGHT));
	bitmap_layer_set_compositing_mode(s_bitmap_layer, GCompOpSet);
	layer_add_child(window_layer, bitmap_layer_get_layer(s_bitmap_layer));

	// Create time TextLayer
	s_time_layer =
		text_layer_create(GRect(0, TIME_Y, bounds.size.w, TIME_HEIGHT));
	...
	s_date_layer =
		text_layer_create(GRect(0, DATE_Y, bounds.size.w, DATE_HEIGHT));
	...
	// Create battery display in top right corner
	int battery_x = bounds.size.w - BATTERY_WIDTH - BATTERY_PADDING_RIGHT;
	int icon_x = battery_x - BATTERY_ICON_SIZE - BATTERY_ICON_GAP;

	s_battery_icon_layer = bitmap_layer_create(
		GRect(icon_x, 0, BATTERY_ICON_SIZE, BATTERY_ICON_SIZE));
	bitmap_layer_set_compositing_mode(s_battery_icon_layer, GCompOpSet);
	layer_add_child(window_layer,
			bitmap_layer_get_layer(s_battery_icon_layer));

	s_battery_layer = text_layer_create(
		GRect(battery_x, 0, BATTERY_WIDTH, BATTERY_HEIGHT));
